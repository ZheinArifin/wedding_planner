...
// mencetak string 
$pdf->Cell(196,7,'Laporan Data Penyewaan',0,1,'C');
...
$pdf->Cell(33,6,'Katalog Produk',1,0,'C');   
...
date_default_timezone_set('Asia/Jakarta');
  $sql = mysqli_query($con,"SELECT * FROM penyewaan WHERE status='Dibayar' OR status='Lunas'");

$no=1;

$pdf->SetFont('Times','',10);
$xc = mysqli_fetch_row(mysqli_query($con, "SELECT COUNT(*) FROM penyewaan WHERE status='Dibayar' OR status='Lunas'"));
$xc = $xc[0];
while ($row = mysqli_fetch_array($sql)){
  $sql1 = $con -> query("SELECT * FROM pelanggan WHERE id='$row[id_pelanggan]'");
  $plg = $sql1 -> fetch_array(MYSQLI_ASSOC); 
  
  $sql2 = $con -> query("SELECT * FROM katalog_produk WHERE id='$row[id_katalog]'");
  ...
  $cellHeight=6; //tinggi sel satu baris normal
    ...
    $pdf->Cell(33,($line * $cellHeight),$produk['katalog_produk'],1,0,'C');
    $pdf->Cell(32,($line * $cellHeight),date('d F Y', strtotime($row['tgl_sewa'])),1,0,'C');
    $xPos=$pdf->GetX();
    $yPos=$pdf->GetY(); 
    $pdf->MultiCell($cellWidth,$cellHeight,$row['alamat'],LRTB,L,0);
    $pdf->SetXY($xPos + $cellWidth , $yPos);
    $pdf->Cell(30,($line * $cellHeight),"Rp. ".number_format($total,0,'.','.'),1,1);
    $ttl += $total;
$no++;
}
    $pdf->SetFillColor(266,266,266);
    ...
    $pdf->Cell(30,6,"Rp. ".number_format($ttl,0,'.','.'),1,1); 
