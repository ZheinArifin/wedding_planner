      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="javascript:void(0)">Diskon Produk</a>
          </div>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title ">Tabel Diskon Produk</h4>
                </div>
                <div class="card-body">
                  <a href="add-diskon.php" class="btn btn-success btn-round">
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tbody>
                        <?php 
                          $no=1;
                          $sql = mysqli_query($con, "SELECT * FROM diskon");
                          while($row = mysqli_fetch_array($sql)){
                          $id_klg = explode(",", $row['id_katalog']);
                          $katalog = array();
                            for ($i=0; $i < count($id_klg); $i++) { 
                              $klg = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM katalog_produk WHERE id = '$id_klg[$i]'"));
                              array_push($katalog, $klg['katalog_produk']);
                            }
                          $dt = implode(", ",$katalog);
                        ?>
                          <tr>
                            ...
                            <td>
                              <a href="edit-diskon.php?id=<?=$row['id'];?>" style="padding:10px; background-color:orange; border-radius:50px">
                              <i class="fas fa-edit"></i></a>
                              &nbsp
                              <a href="javascript:void(0)" onclick="hapus_diskon('<?=$row['id'];?>')" style="padding:10px; background-color:red; border-radius:50px">
...
<script>
  function hapus_diskon(x){
    $.ajax({
      url : "../proses/delete_diskon.php",
      type: "POST",
      data:{
        id: x,
      },
